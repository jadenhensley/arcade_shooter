    ...
    def update(self):
        ...
        if self.SPACE:
            if self.lasercount < self.max_lasers:
                e.add(PlayerLaser())
                self.lasercount += 1
                print(self.lasercount)
            elif self.lasercount == self.max_lasers:
                self.lastTick = pygame.time.get_ticks()
                self.lasercount += 1
                print(self.lasercount)
        
        if pygame.time.get_ticks() > self.lastTick + 1000 and self.SPACE == True:
            self.lasercount = 0
            # cannot spam laser
...
class Game:
    ...
    def game(self):
        ...
        screen.fill((40,40,40))
        printgLarge("In Game", sWidth // 4, sHeight // 2, (0,255,0))
        p.update()
        e.update()
        clock.tick(60)
        pygame.display.update()
